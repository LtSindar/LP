...
/*START*/ 
    this.initIframe = function(frame_link){
        ...
        $(window).trigger('AuthFrame_loaded');
    }
/*END*/ 
...
/*END*/
    ...
    this.closePopup = function() {
        $(".b-popup").fadeOut();
        $(".b-popups__auth-frame").not(".b-popups__frame_no-close").fadeOut();
    };
...
/*START*/   
            ...
            //ХАК::Если пользователь САМ ничего не выбрал, то катимся на слайдер
            //За аксиому берем, что если на странице есть слайдер, то выбор происходит в нем (или выбор в нем более приоритетен)
            if(!NP.target && $(".b-slider").size() > 0){
                $("body,html").animate({
                    scrollTop: $(".b-slider").offset().top - 40 + "px"
                });
                return;
            ...
            NP.frame.isAuth(//Auth
                ...
                }($(this).data("popup"),'b-popups__auth-frame')
            );

/*END*/   

//            if( $(".b-popups__auth-frame").size() > 0 ) {
//                $(".b-popups__auth-framee")[0].contentWindow.showPopup($(this).data("popup"));
//                $(".b-popups__auth-frame").fadeIn();
//            }
        });
    })();
/*START*/   
    this.bindOnAuth = function() {

        //Вешаем бинд на событие авторизации в фрейме авторизации, если есть блок, помеченный специальным классом
        if($('.auto-bind-auth--js').size() < 1) return false;
        
        var authTargetUrl = $('.auto-bind-auth--js').data('auth-target-url');
        if(authTargetUrl) NP.target = authTargetUrl;
        ...
        if(!NP.frame.bindOnAuth || typeof NP.frame.bindOnAuth != 'function'){
            console.log('Не доступна авторизация');
            return;
        } 
        
        NP.frame.bindOnAuth(function(NP){
                                return function(t){
                                    NP.go(t);
                                }
                            }(NP));
...
/*START*/     
    $(window).bind('AuthFrame_loaded',function(e){
                    console.log('catch AuthFrame_loaded');
                    window.NP.bindOnAuth();
                });
/*END*/ 
